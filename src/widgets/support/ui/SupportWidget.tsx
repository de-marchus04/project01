...
export const SupportWidget = () => {
  ...
  const loadMessages = async () => {
    ...
    const unread = userMsgs.filter(
      (m) => (m.status === 'replied' || m.status === 'bot_answered') && !m.readByUser,
    ).length;
                ...
                <div key={msg.id} className="mb-4">
                  ...
                  <div className="d-flex justify-content-end mb-2">
                    <div
                      ...
                      style={{ backgroundColor: 'var(--color-primary)', color: 'white', maxWidth: '85%' }}
                    >
                      <small className="d-block fw-bold mb-1" style={{ opacity: 0.8 }}>
                        Вы: {msg.questionType !== 'Написать свой вопрос' ? msg.questionType : ''}
                      </small>
                      {msg.message}
                    </div>
                  </div>
                  ...
                  {msg.status === 'new' && (
                    <div className="d-flex justify-content-start">
                      <div
                        className="p-2 rounded-3 bg-white border shadow-sm text-muted"
                        style={{ maxWidth: '85%', fontStyle: 'italic' }}
                      >
