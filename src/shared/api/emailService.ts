...
export const emailService = {
  ...
  sendOrderConfirmation: async (email: string, productName: string, price: number, notes: string | undefined, profileUrl: string): Promise<void> => {
  ...
  sendAdminOrderNotification: async (email: string, productName: string, buyer: string, price: number, notes: string | undefined, adminUrl: string): Promise<void> => {
    const notesLine = notes ? `\n\nЖелаемая дата/время: ${notes}` : '';
    await emailService.sendEmail(
      email,
