...
describe("getOrders", () => {
  ...
  it("returns mapped orders for admin", async () => {
    ...
    (mockPrisma.order.findMany as jest.Mock).mockResolvedValue([
      {
        id: "o1", productName: "Yoga Course", amount: 99.99,
        status: "PENDING", createdAt: new Date("2025-06-15"),
        itemId: "c1", itemType: "COURSE", user: { username: "alice" },
        guestName: null, userId: "u1", notified: false,
      },
      {
        id: "o2", productName: null, amount: 50,
        status: "COMPLETED", createdAt: new Date("2025-06-14"),
        itemId: "t1", itemType: "TOUR", user: null,
        guestName: "Guest Bob", userId: null, notified: true,
      },
    ]);
