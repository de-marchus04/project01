...
describe("getItemReviews", () => {
  ...
  it("calculates average and distribution correctly", async () => {
    (mockPrisma.review.findMany as jest.Mock).mockResolvedValue([
      { id: "r1", userId: "u1", itemId: "c1", itemType: "COURSE", rating: 5, text: "Great", createdAt: new Date(), user: { username: "alice", avatar: null } },
      { id: "r2", userId: "u2", itemId: "c1", itemType: "COURSE", rating: 3, text: null, createdAt: new Date(), user: { username: "bob", avatar: null } },
    ]);
...
describe("getUserReview", () => {
  ...
  it("returns review data when found", async () => {
    ...
    (mockPrisma.review.findUnique as jest.Mock).mockResolvedValue({
      id: "r1", userId: "u1", itemId: "c1", itemType: "COURSE",
      rating: 5, text: "Amazing", createdAt: new Date("2025-01-01"),
