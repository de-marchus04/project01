    ...
    <main>
      ...
      <section className="py-5" style={{ backgroundColor: 'var(--color-bg)' }}>
        <div className="container py-5">
          <SectionHeader
            badge={tStr('Избранное')}
            title={tStr('Список желаний')}
            observe={observe}
          />
            ...
            <div className="text-center py-5 reveal-up" ref={observe as any}>
              <div className="mb-4" style={{ opacity: 0.15 }}>
                <svg width="120" height="120" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M60 100C60 100 15 72 15 42C15 28 26 18 38 18C47 18 55 24 60 32C65 24 73 18 82 18C94 18 105 28 105 42C105 72 60 100 60 100Z" stroke="var(--color-primary)" strokeWidth="3" fill="none"/>
                  <path d="M40 55C40 55 50 65 60 75C70 65 80 55 80 45C80 39 75 35 70 35C65 35 62 38 60 42C58 38 55 35 50 35C45 35 40 39 40 45" stroke="var(--color-primary)" strokeWidth="2" fill="none" strokeLinecap="round"/>
                </svg>
              </div>
              <h3 className="font-playfair mb-3" style={{ color: 'var(--color-text)' }}>{tStr('Список пуст')}</h3>
            ...
            <div className="row g-4">
              {items.map((item, idx) => (
                <div
                  ...
                  ref={observe as any}
                >
                  <div className="card border-0 shadow-sm rounded-4 overflow-hidden h-100" style={{ backgroundColor: 'var(--color-card-bg)' }}>
                    {item.imageUrl ? (
                      ...
                      <div className="d-flex align-items-center justify-content-center" style={{ height: '180px', backgroundColor: 'var(--color-secondary)' }}>
                        <i className="bi bi-image" style={{ fontSize: '3rem', color: 'var(--color-text-muted)' }}></i>
                      </div>
                    )}
                    <div className="card-body p-4 d-flex flex-column">
                      <div className="mb-2">
                        <span className="badge rounded-pill px-3 py-2" style={{ backgroundColor: 'var(--color-secondary)', color: 'var(--color-primary)', fontSize: '0.75rem', fontWeight: 600 }}>
                          {TYPE_LABEL[item.itemType] || item.itemType}
                        </span>
                      </div>
                      <h5 className="fw-bold font-playfair mb-2 flex-grow-1" style={{ color: 'var(--color-text)', lineHeight: '1.35' }}>
                        {item.title}
                      </h5>
