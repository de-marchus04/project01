...
export default function CoursesBackClient({ initialData }: { initialData: PaginatedResponse<Course> }) {
  ...
  useEffect(() => {
    async function loadProducts() {
      setLoading(true);
      try {
        const response = await getBackCoursesPaginated(currentPage, 6, searchQuery, sortBy);
  ...
  return (
    <main>
      <section className="hero-section page-hero d-flex align-items-center text-center text-white position-relative"
        ...
        <HeroSlider pageKey="courses-back" images={[
        ...
        <div className="container position-relative z-2">
          ...
          <p className="lead mb-5 col-lg-8 mx-auto fw-light">{t.programs.heroBackDesc}</p>
        </div>
      </section>
