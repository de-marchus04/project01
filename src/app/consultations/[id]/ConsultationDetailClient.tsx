      ...
      <main>
        <section
          ...
          style={{
            height: '55vh',
            minHeight: '420px',
            background: `linear-gradient(rgba(62,66,58,0.55),rgba(62,66,58,0.75)),url('${heroImg}') center/cover`,
          }}
        >
          <div className="container position-relative z-2 pb-5">
            ...
            <h1 className="display-4 font-playfair mb-3" style={{ textShadow: '0 4px 15px rgba(0,0,0,0.2)' }}>
              {heroTitle}
            </h1>
            <p className="lead col-lg-7 fw-light mb-0" style={{ textShadow: '0 2px 10px rgba(0,0,0,0.2)' }}>
              {heroDesc}
            </p>
          </div>
        </section>
        ...
        <section className="py-5" style={{ backgroundColor: 'var(--color-bg)' }}>
          <div className="container py-3 text-center">
            ...
            >
              ...
              <h4 className="font-playfair fw-bold mb-3" style={{ color: 'var(--color-text)' }}>
                {t.programs.consultTitleNum}
              </h4>
              <p className="text-muted mb-4">{t.programs.consultDescNum}</p>
              <a
                href={backHref}
                className="btn rounded-pill px-5 py-2 fw-semibold me-2 mb-2"
        ...
        <div className="container py-5">
          <div className="row g-5">
            <div className="col-lg-8">
              <h3 className="font-playfair mb-4" style={{ color: 'var(--color-text)' }}>
                {t.programs.aboutCourse}
              </h3>
              <div className="text-muted mb-4" style={{ lineHeight: '1.8', whiteSpace: 'pre-wrap' }}>
                {localized_course.fullDescription ||
                  `${t.courseDetail.desc1} ${t.courseDetail.desc2} ${t.courseDetail.desc3}`}
              </div>

              <h4 className="font-playfair mt-5 mb-4" style={{ color: 'var(--color-text)' }}>
                {t.programs.whatAwaits}
              </h4>
              <ul className="list-unstyled text-muted">
                {localized_course.features &&
                Array.isArray(localized_course.features) &&
                localized_course.features.length > 0 ? (
                  ...
                  (localized_course.features as any)
                    .split('\n')
            ...
            <div className="col-lg-4">
              <div
                ...
                style={{ top: '100px', borderRadius: '20px', backgroundColor: 'var(--color-card-bg)' }}
              >
                <h4 className="font-playfair mb-4" style={{ color: 'var(--color-text)' }}>
                  {t.programs.programDetails}
                </h4>
                <div className="d-flex align-items-center gap-3 mb-3 text-muted">
                  <i className="bi bi-clock fs-5"></i>
                  <div>
                    <small className="d-block text-uppercase" style={{ fontSize: '0.7rem', letterSpacing: '1px' }}>
                      {t.courseDetail.durationLabel}
                    </small>
                    <span className="fw-bold" style={{ color: 'var(--color-text)' }}>
                      {t.courseDetail.durationVal}
                    </span>
                  </div>
                </div>
                <div className="d-flex align-items-center gap-3 mb-3 text-muted">
                  ...
                  <div>
                    <small className="d-block text-uppercase" style={{ fontSize: '0.7rem', letterSpacing: '1px' }}>
                      {t.courseDetail.formatLabel}
                    </small>
                    <span className="fw-bold" style={{ color: 'var(--color-text)' }}>
                      {t.courseDetail.formatVal}
                    </span>
                  </div>
                </div>
                <div className="d-flex align-items-center gap-3 mb-4 text-muted">
                  ...
                  <div>
                    <small className="d-block text-uppercase" style={{ fontSize: '0.7rem', letterSpacing: '1px' }}>
                      {t.courseDetail.accessLabel}
                    </small>
                    <span className="fw-bold" style={{ color: 'var(--color-text)' }}>
                      {t.courseDetail.accessVal}
                    </span>
                  </div>
                </div>
                <div className="d-flex align-items-center gap-3 mb-4 text-muted">
                  {localized_course.authorPhoto ? (
                    <Image
                      ...
                      src={
                        localized_course.authorPhoto ||
                        'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=24'
                      }
                      style={{ objectFit: 'cover', borderRadius: '50%' }}
                      alt={localized_course.author || t.courseDetail.authorLabel}
                    />
                  ) : (
                  ...
                  <div>
                    <small className="d-block text-uppercase" style={{ fontSize: '0.7rem', letterSpacing: '1px' }}>
                      {t.courseDetail.authorLabel}
                    </small>
                    <span className="fw-bold" style={{ color: 'var(--color-text)' }}>
                      {localized_course.author || t.courseDetail.authorDef}
                    </span>
                  </div>
                </div>
                <hr className="my-4" />
                <div className="text-center">
                  {promoPrice !== null ? (
                    <div className="mb-3">
                      ...
                      <span className="fs-3 fw-bold" style={{ color: 'var(--color-primary)' }}>
                        {formatPrice(promoPrice, lang)}
                      </span>
                    </div>
                  ) : (
                    <span className="d-block fs-3 fw-bold mb-3" style={{ color: 'var(--color-text)' }}>
                      {formatPrice(localized_course.price, lang)}
                    </span>
                  )}
                  <PromoCodeInput
