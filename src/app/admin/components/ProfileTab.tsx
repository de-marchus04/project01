...
export default function ProfileTab({ session, updateSession, showToast, onProfileStatusChange, onProfileSaved, initialProfile }: Props) {
  const { t } = useLanguage();
  ...
  useEffect(() => { setAdminProfile(initialProfile); }, [initialProfile]);
  ...
  const handleProfileSave = async (e: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      ...
      await updateMyProfile(username, {
        name: adminProfile.name, email: adminProfile.email,
        phone: adminProfile.phone, avatar: adminProfile.photoUrl,
      });
      await updateSession();
      ...
      setTimeout(() => { setIsProfileSaved(false); onProfileStatusChange(false, false); }, 3000);
    ...
    reader.onloadend = () => {
      ...
      img.onload = () => {
        ...
        uploadImageToCloud(dataUrl)
          ...
          .catch(err => { console.error('Cloudinary fallback', err); setEdited({ ...adminProfile, photoUrl: dataUrl }); });
      };
      ...
      <div>
        <div className="row g-3">
          <div className="col-md-6">
            ...
            <input type="text" className="form-control" value={adminProfile.name} onChange={(e) => setEdited({ ...adminProfile, name: e.target.value })} />
          </div>
          <div className="col-md-6">
            <label className="form-label fw-bold">{t.admin.profileEmail}</label>
            <input type="email" className="form-control" value={adminProfile.email} onChange={(e) => setEdited({ ...adminProfile, email: e.target.value })} />
          </div>
          <div className="col-md-6">
            <label className="form-label fw-bold">{t.admin.profilePhone}</label>
            <input type="tel" className="form-control" value={adminProfile.phone} onChange={(e) => setEdited({ ...adminProfile, phone: e.target.value })} />
          </div>
          <div className="col-md-6">
            <label className="form-label fw-bold">{t.admin.profilePhoto} <span className="text-danger">*</span></label>
            <div className="d-flex gap-2">
              <input type="text" className="form-control" placeholder={t.admin.profilePhotoPlaceholder} value={adminProfile.photoUrl} onChange={(e) => setEdited({ ...adminProfile, photoUrl: e.target.value })} />
          ...
          <div className="col-12 mt-4 d-flex align-items-center gap-3">
            ...
            {isProfileSaved && (
              <div className="d-flex align-items-center fw-medium" style={{ color: '#198754', animation: 'fadeIn 0.3s ease' }}>
                <i className="bi bi-check-circle-fill me-2 fs-5"></i>{t.admin.profileChangesApplied}
              </div>
            )}
