...
export default function ToursTab({ adminProfile, showToast }: Props) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteTour(id); await loadTours(); }
          ...
          <div className="table-responsive">
            <table className="table table-hover align-middle">
              ...
              <tbody>
                ...
                {tours.length === 0 && <tr><td colSpan={6} className="text-center py-4" style={{ color: 'var(--color-text-muted)' }}>{t.admin.noData}</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
