...
export default function TestimonialsTab({ showToast }: Props) {
  ...
  const loadTestimonials = async () => {
    try { setTestimonials(await getTestimonials()); } catch (e) { console.error(e); }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteTestimonial(id); await loadTestimonials(); }
          ...
          <form onSubmit={handleSave}>
            <div className="row g-3">
              ...
              <div className="col-md-12">
                ...
                <input type="text" name="course" className="form-control" required defaultValue={editingItem?.course || ''} />
              </div>
              <div className="col-md-12">
                <label className="form-label">{t.admin.formReview}</label>
                <textarea name="description" className="form-control" rows={3} required defaultValue={editingItem?.text || ''}></textarea>
              </div>
          ...
          <div className="table-responsive">
            <table className="table table-hover align-middle">
              <thead className="table-light">
                <tr><th>ID</th><th>{t.admin.colName}</th><th>{t.admin.colCourse}</th><th>{t.admin.colReview}</th><th className="text-end">{t.admin.colActions}</th></tr>
              </thead>
              <tbody>
                {testimonials.map(item => (
                  <tr key={item.id}>
                    <td>{item.id}</td><td>{item.name}</td><td>{item.course}</td>
                    <td className="text-truncate" style={{ maxWidth: '200px' }}>{item.text}</td>
                ...
                {testimonials.length === 0 && <tr><td colSpan={5} className="text-center py-4" style={{ color: 'var(--color-text-muted)' }}>{t.admin.noData}</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
