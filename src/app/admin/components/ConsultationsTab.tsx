...
import { getAllAdminConsultations, addConsultation, updateConsultation, deleteConsultation } from "@/shared/api/consultationApi";
...
export default function ConsultationsTab({ adminProfile, showToast }: Props) {
  ...
  const loadConsultations = async () => {
    try { setConsultations(await getAllAdminConsultations()); } catch (e) { console.error(e); }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteConsultation(id); await loadConsultations(); }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const consultationData = await translateObjectFields({
          ...
          <div className="table-responsive">
            <table className="table table-hover align-middle">
              ...
              <tbody>
                ...
                {consultations.length === 0 && <tr><td colSpan={4} className="text-center py-4" style={{ color: 'var(--color-text-muted)' }}>{t.admin.noData}</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
