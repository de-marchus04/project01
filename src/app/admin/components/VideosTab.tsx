...
import { getVideos, addVideo, updateVideo, deleteVideo } from "@/shared/api/blogApi";
...
export default function VideosTab({ showToast }: Props) {
  const { t } = useLanguage();
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const videoData = await translateObjectFields({
        title: data.title, description: data.description,
        videoUrl: data.videoUrl, thumbnailUrl: data.imageUrl, tag: data.tag,
      });
            ...
            <div className="row g-3">
              ...
              <div className="col-md-6">
                ...
                <div className="d-flex gap-2">
                  <input type="text" id="mainImageUrlInput" name="imageUrl" className="form-control" required defaultValue={editingItem?.thumbnailUrl || ''} />
          ...
          <div className="table-responsive">
            <table className="table table-hover align-middle">
              <thead className="table-light">
                <tr><th>ID</th><th>{t.admin.colTitle}</th><th>{t.admin.colTag}</th><th className="text-end">{t.admin.colActions}</th></tr>
              </thead>
              <tbody>
                {videos.map(item => (
                  <tr key={item.id}>
                ...
                {videos.length === 0 && <tr><td colSpan={4} className="text-center py-4" style={{ color: 'var(--color-text-muted)' }}>{t.admin.noData}</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
