...
export default function SlideshowTab({ showToast }: Props) {
  ...
  useEffect(() => { loadSlides(); }, [selectedPage]);
  ...
  const handleAddVideo = async () => {
    ...
    try {
      await createPageSlide({ pageKey: selectedPage, url: videoUrl.trim(), mediaType: "VIDEO", sortOrder: slides.length });
      await loadSlides();
      setVideoUrl("");
    ...
    >
      ...
      <div className="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        ...
        <select
          ...
          style={{ maxWidth: 280, border: "1px solid var(--color-border)", backgroundColor: "var(--color-card-bg)", color: "var(--color-text)" }}
          value={selectedPage}
        ...
        <div className="d-flex gap-2 flex-wrap">
          <label
            ...
            style={{ backgroundColor: "var(--color-primary)", color: "#fff", border: "none", cursor: imgUploading ? "default" : "pointer", opacity: imgUploading ? 0.6 : 1 }}
          >
            <i className="bi bi-image me-2"></i>
            {imgUploading ? t.admin.slideshowUploading : t.admin.slideshowAddImage}
            <input type="file" accept="image/*" className="d-none" disabled={imgUploading} onChange={handleImageUpload} />
          </label>
      ...
      {/* Video URL form */}
      {showVideoForm && (
        <div className="card border-0 p-3 mb-4 rounded-3" style={{ background: "var(--color-secondary)", border: "1px solid var(--color-border)" }}>
          ...
          <div className="d-flex gap-2">
            <input
              ...
              style={{ border: "1px solid var(--color-border)", backgroundColor: "var(--color-card-bg)", color: "var(--color-text)" }}
            />
            <button
          ...
          {slides.map(slide => (
            <div key={slide.id} className="col-12 col-md-6 col-xl-4">
              <div
                ...
                style={{ borderRadius: "12px", overflow: "hidden", border: "1px solid var(--color-border)", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", opacity: slide.active ? 1 : 0.5 }}
              >
                {/* Preview */}
                <div style={{ position: "relative", paddingTop: "56.25%", backgroundColor: "var(--color-secondary)" }}>
                  ...
                  <span
                    ...
                    style={{ top: 8, left: 8, backgroundColor: slide.mediaType === "VIDEO" ? "rgba(99,102,241,0.85)" : "rgba(0,0,0,0.55)", backdropFilter: "blur(4px)", borderRadius: "6px", fontSize: "0.7rem" }}
                  >
                ...
                <div className="p-3">
                  ...
                  <div className="d-flex align-items-center gap-2 mb-2">
                    ...
                    <input
                      ...
                      style={{ width: "64px", border: "1px solid var(--color-border)", backgroundColor: "var(--color-card-bg)", color: "var(--color-text)" }}
                  ...
                  {/* URL truncated */}
                  <div className="small text-truncate mb-2" style={{ color: "var(--color-text-muted)", fontSize: "0.72rem" }}>
                    {slide.url}
                  </div>
                  ...
                  <div className="d-flex gap-2">
                    <button
                      ...
                      style={{ fontSize: "0.78rem", border: `1px solid ${slide.active ? "var(--color-primary)" : "var(--color-border)"}`, color: slide.active ? "var(--color-primary)" : "var(--color-text-muted)", backgroundColor: "transparent" }}
                    ...
                    <button
                      ...
                      style={{ fontSize: "0.78rem", border: "1px solid #dc3545", color: "#dc3545", backgroundColor: "transparent" }}
