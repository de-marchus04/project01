...
export default function FaqsTab({ showToast }: Props) {
  ...
  const loadFaqs = async () => {
    try { setFaqs(await getFAQs()); } catch (e) { console.error(e); }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteFAQ(id); await loadFaqs(); }
          ...
          <form onSubmit={handleSave}>
            <div className="row g-3">
              <div className="col-md-12">
                ...
                <input type="text" name="title" className="form-control" required defaultValue={editingItem?.question || ''} />
              </div>
              <div className="col-md-12">
                <label className="form-label">{t.admin.formAnswer}</label>
                <textarea name="description" className="form-control" rows={3} required defaultValue={editingItem?.answer || ''}></textarea>
              </div>
          ...
          <div className="table-responsive">
            <table className="table table-hover align-middle">
              <thead className="table-light">
                <tr><th>ID</th><th>{t.admin.colQuestion}</th><th>{t.admin.colAnswer}</th><th className="text-end">{t.admin.colActions}</th></tr>
              </thead>
              <tbody>
                {faqs.map(item => (
                  <tr key={item.id}>
                    <td>{item.id}</td><td>{item.question}</td>
                    <td className="text-truncate" style={{ maxWidth: '200px' }}>{item.answer}</td>
                ...
                {faqs.length === 0 && <tr><td colSpan={4} className="text-center py-4" style={{ color: 'var(--color-text-muted)' }}>{t.admin.noData}</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
