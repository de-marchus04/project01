        ...
        <div className="container py-5">
          ...
          <div className="row g-4">
            ...
            {!loading && tours.map((tour, index) => {
              ...
              return (
                <div key={loc_tour.id} className={`col-lg-6 reveal-up${index > 0 ? ` reveal-delay-${index % 3}` : ''}`} ref={observe as any}>
                  <div className="card border-0 h-100 overflow-hidden" style={{ borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
                    <div className="row g-0 h-100">
                      <div className="col-md-5">
                        <img
                          src={loc_tour.imageUrl || "https://images.unsplash.com/photo-1537996194471-e657df975ab4?q=80&w=800"}
                          className="img-fluid h-100 w-100"
                          style={{ objectFit: 'cover', minHeight: '250px', borderLeft: '4px solid var(--color-accent)' }}
                          alt={loc_tour.title}
                          onError={(e) => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1552196563-55cd4e45efb3?q=80&w=800&auto=format&fit=crop"; }}
                        />
                      </div>
                        ...
                        <div className="card-body d-flex flex-column justify-content-center h-100 p-4">
                          <div className="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-2">
                            ...
                            <small className="text-muted">
                              <i className="bi bi-geo-alt-fill me-1" style={{ color: 'var(--color-accent)' }}></i>{loc_tour.location}
                            </small>
                          </div>
                          <div className="d-flex align-items-start justify-content-between mb-2 gap-2">
                            ...
                            <span className="fw-bold text-nowrap" style={{ color: 'var(--color-primary)' }}>{formatPrice(loc_tour.price, lang)}</span>
                          </div>
                          <p className="card-text text-muted mb-4" style={{ fontSize: '0.95rem' }}>
                            {loc_tour.description}
                          </p>
                          <div className="mt-auto">
                            <Link href={`/tours/${loc_tour.id}`} className="btn btn-outline-primary-custom rounded-pill px-4">
                              {tStr("Подробнее")}
                            </Link>
                          </div>
                        </div>
