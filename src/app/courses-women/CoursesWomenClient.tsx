...
export default function CoursesWomenClient({ initialData }: { initialData: PaginatedResponse<Course> }) {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    async function loadProducts() {
      setLoading(true);
      try {
        const response = await getWomenCoursesPaginated(currentPage, 6, searchQuery, sortBy);
  ...
  return (
    <main>
      <section className="hero-section page-hero d-flex align-items-center text-center text-white position-relative"
        ...
        <HeroSlider pageKey="courses-women" images={[
        ...
        <div className="container position-relative z-2">
          ...
          <h1 className="display-3 font-playfair mb-4" style={{ textShadow: '0 4px 15px rgba(0,0,0,0.2)' }}>{t.programs.heroWomenTitle}</h1>
          <p className="lead mb-5 col-lg-8 mx-auto fw-light">{t.programs.heroWomenDesc}</p>
        </div>
      </section>
