    ...
    <div className="card h-100 shadow border-0 hover-scale-sm transition-all">
      ...
      <div className="card-body d-flex flex-column">
        <Link href={detailUrl} className="text-decoration-none">
          <h5 className="card-title font-playfair fw-bold mb-3" style={{ color: 'var(--color-text)' }}>{localized_course.title}</h5>
        </Link>
        <p className="card-text text-muted small flex-grow-1">{localized_course.description}</p>
        <div className="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
          <span className="fs-4 fw-bold" style={{ color: 'var(--color-text)' }}>{formatPrice(localized_course.price, lang)}</span>
          <button
